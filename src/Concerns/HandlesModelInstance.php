...
trait HandlesModelInstance
{
    ...
    /**
     * Get field definition from the model type schema.
     *
     ...
     * @return array|null
     */
    protected function getFieldDefinitionFromSchema($fieldId, $modelTypeId): ?array
    {
        // Get model type
        $modelType = ModelType::find($modelTypeId);
        if ( ! $modelType || ! $modelType->model_type_schema) {
            return null;
        }


        // Search for the field in all sections
        foreach ($modelType->model_type_schema as $section) {
            if ( ! isset($section['Fields']) || ! is_array($section['Fields'])) {
                continue;
            }

            foreach ($section['Fields'] as $field) {
                if (isset($field['custom_id']) && $field['custom_id'] === $fieldId) {

                    // Ensure the field has all necessary attributes
                    if ( ! isset($field['multiple']) && isset($field['type']) && $field['type'] === 'select') {
                        // Check if there's an 'extra' property that might contain multiple
                        if (isset($field['extra'], $field['extra']['multiple'])) {
                            $field['multiple'] = $field['extra']['multiple'];
                        }

                        // Check if there's a searchable property
                        if ( ! isset($field['searchable']) && isset($field['extra'], $field['extra']['searchable'])) {
                            $field['searchable'] = $field['extra']['searchable'];
                        }
                    }
    ...
    protected function createOrUpdateModelInstance(): void
    {
        ...
        foreach ($modelTypeSchema as $sectionIndex => $section) {
            ...
            foreach ($section['Fields'] ?? [] as $fieldIndex => $field) {
                ...
                $field = $this->getFieldDefinitionFromSchema($customId, $modelTypeID);
                if ( ! $field) {
                    continue;
                }
                ...
                if ($fieldType === 'select' && $isRequired && ($value === null || $value === '')) {
